      ...
      if (dataScript) {
        ...
        if (raw && raw !== '{}' && raw.length > 5) {
          ...
          if (parsed.fields && Object.keys(parsed.fields).length > 0) {
            // ⚠️ 关键校验：检查 pe-saved-data 里的 name 是否和 HTML 静态内容一致
            // 防止 AI 生成新文件时把旧用户的数据复制进来导致内容错乱
            ...
            if (nameMatch) {
              ...
              console.warn('PE: pe-saved-data name不匹配（saved:', savedName, 'html:', htmlName, '），忽略旧数据');
            }
          }
        }
      }
        ...
        if (lsRaw) {
          ...
          if (lsParsed && Object.keys(lsParsed).length > 0) {
            ...
            if (lsPlaceholders.includes(htmlName2) || !lsName || lsName === htmlName2) {
              ...
              console.warn('PE: localStorage name不匹配，忽略');
            }
          }
        }
      } catch(e) {}
